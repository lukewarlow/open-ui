...
The `openable` attribute is allowed on all HTML elements.
There are situations where it may not make sense but the same is true of `popover` and that works on all HTML elements.
Author guidance can be provided that `openable` on your `<body>` or `<html>` element is not a good idea.
